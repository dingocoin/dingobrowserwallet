...
- **`source/dingocoin.js`**: all chain and crypto logic, in CommonJS.
  ...
  - It uses Node's `crypto` and `Buffer`. Webpack 5 doesn't polyfill these, so `webpack.config.js` maps them to `crypto-browserify`, `buffer`, and related packages through `resolve.fallback` and `ProvidePlugin`. In the browser, `secp256k1` falls back to `elliptic`.
