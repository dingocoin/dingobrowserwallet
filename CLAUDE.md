...
- Releases come from `.github/workflows/release.yml`, which runs manually on `master` (`workflow_dispatch`). It refuses a version that is already tagged or has a release or draft. It runs lint, typecheck and tests, builds Chrome, runs `scripts/package-release.mjs`, and creates a **draft** GitHub Release `v<version>` with the zip and `SHA256SUMS`; publishing the draft creates the tag.
  ...
  - **Firefox:** the release attaches the xpi and the source archive, but uploading them to AMO is manual, and a permanent install needs Mozilla's signature. `npm run package:source` builds `extension/source/dingocoin-wallet-<version>-source.zip` from `git archive HEAD`, so it refuses a dirty tree and refuses a version mismatch between `package.json` and `source/manifest.json`. AMO requires the sources for every version, because the add-on ships bundled and minified code, and requires that they rebuild the upload **exactly** — so never attach an archive from a different commit than the xpi. Reproducibility holds for the files inside the xpi, not for the xpi itself: the zip container records timestamps, so compare `extension/firefox/` trees rather than xpi checksums. `HtmlWebpackPlugin`'s `hash: true` used to break even that, by stamping webpack's build-directory-dependent compilation hash into the generated HTML; don't reintroduce it. Reviewer notes live in `.github/AMO_REVIEWER_NOTES.md`; the build steps a reviewer follows are in `README.md`. **Uploading to AMO stays a manual step, deliberately — don't automate it.** Doing so means holding a credential in CI that can publish a new version to every installed wallet, and a leak would ship arbitrary code to people's funds. Releases are infrequent and the submission needs a human watching for reviewer questions anyway.
...
- Firefox needs different manifest keys, which are vendor-prefixed:
  ...
  - **Add-on ID:** Firefox's ID lives in `browser_specific_settings.gecko`. The MV2 `applications` key is invalid in MV3, and without the ID `storage.sync` fails for temporary add-ons. The ID is `{511034e7-634f-47bd-a15d-d47adfa28c55}`. **Do not restore the old `{186b9e06-824a-402c-b652-897f3304f5d5}`**, which came from the 2022 template import: a long-dead Firefox submission still holds it on AMO, so uploading under it fails with "Duplicate add-on ID found", and Mozilla never releases a GUID even after the add-on is deleted. Changing the ID was safe because this extension has never shipped on Firefox; note that `storage.sync` is keyed to the ID, so an ID change orphans the accounts and vault of any existing install.
  - **Data collection:** `browser_specific_settings.gecko.data_collection_permissions` is `{"required": ["financialAndPaymentInfo"]}`. Mozilla made the key mandatory for new extensions on 2025-11-03, and addons-linter errors without it. `financialAndPaymentInfo` is the honest value because `provider.js` sends address scripthashes to third-party ElectrumX servers and broadcasts transactions there, and `Background/index.ts` gives any page the active address; Mozilla counts anything leaving the local browser as collection. The wallet has no analytics or telemetry, so nothing goes in `optional`. Declaring the key on `strict_min_version` 109 produces two `KEY_FIREFOX_UNSUPPORTED_BY_MIN_VERSION` warnings, because support landed in Firefox 140 (Android 142); older Firefox ignores the key. Raising the minimum to 140 would clear them at the cost of 109-139.
